...
class Taxonomy_Resource {
    ...
    private $prefix_name = 'option_category_resource_';

    public function __construct() {
        add_action('init', array($this, 'create_taxonomy'));

        add_action('resource_category_add_form_fields', array($this, 'add_form'));
        add_action('resource_category_edit_form_fields', array($this, 'edit_form'));
        ...
        add_action('create_resource_category', array($this, 'save_option'));
        add_action('edited_resource_category', array($this, 'save_option'));
        add_action('delete_resource_category', array($this, 'delete_option'));
    }
    ...
    public function edit_form($term) {
        ...
        ?>
        <input  type="hidden" name="cate_cn" id="cate_cn" value="<?php echo $arr_value['cate_resource_cn']; ?>" />

        <tr class="form-field">
            <th scope="row" valign="top">  <label for="cate_vn"> <?php _e('Name') ?> (<?php _e('Vietnamese') ?> )</label></th>
            <td><input  type="text" name="cate_vn" id="cate_vn" value="<?php echo $arr_value['cate_resource_vn']; ?>" /></td>
        </tr>
        <tr class="form-field">
            <th scope="row" valign="top">   <label for="cate_en"><?php _e('Name') ?> ( <?php _e('English') ?> )</label> </th>
            <td>    <input type="text" name="cate_en" id="cate_en" value="<?php echo $arr_value['cate_resource_en']; ?>" /></td>
        </tr>
        <tr class="form-field">
            <th scope="row" valign="top">   <label for="cate_en">   <?php _e('Show Order') ?></label> </th>
            <td>    <input type="text" name="cate_order" id="cate_order" value="<?php echo $arr_value['cate_resource_order']; ?>" /></td>
        </tr>
    ...
    public function save_option($term_id) {
        $arr = array(
            'cate_resource_cn' => $_POST['cate_cn'],
            'cate_resource_vn' => $_POST['cate_vn'],
            'cate_resource_en' => $_POST['cate_en'],
            'cate_resource_order' => $_POST['cate_order'],
        );
...
//            'description' => __('Description'),
    ...
    public function category_columns_manage($out, $column_name, $theme_id) {
        $theme = get_term($theme_id, 'resource_category');

        $strOption = get_option($this->prefix_name . $theme->term_id);
        switch ($column_name) {
            case 'order':
                echo isset($strOption['cate_resource_order']) ? $strOption['cate_resource_order'] : '0';
                break;
            case 'vietnamese':
                echo isset($strOption['cate_resource_vn']) ? $strOption['cate_resource_vn'] : '';
                break;
            case 'english':
                echo isset($strOption['cate_resource_en']) ? $strOption['cate_resource_en'] : '';
                break;
            default:
                break;
        }
